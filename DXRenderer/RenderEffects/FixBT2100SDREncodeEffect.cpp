...
#include "pch.h"
#include <initguid.h>
#include "FixBT2100SDREncodeEffect.h"
#include "..\Common\BasicReaderWriter.h"

#define XML(X) TEXT(#X)

FixBT2100SDREncodeEffect::FixBT2100SDREncodeEffect() :
    m_refCount(1),
    m_constants{}
{
}

HRESULT __stdcall FixBT2100SDREncodeEffect::CreateFixBT2100SDREncodeImpl(_Outptr_ IUnknown** ppEffectImpl)
{
    // Since the object's refcount is initialized to 1, we don't need to AddRef here.
    *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) FixBT2100SDREncodeEffect());
...
HRESULT FixBT2100SDREncodeEffect::Register(_In_ ID2D1Factory1* pFactory)
{
    ...
    PCWSTR pszXml =
        XML(
            ...
            <Effect>
                <!-- System Properties -->
                <Property name='DisplayName' type='string' value='FixBT2100SDREncode'/>
    ...
    return pFactory->RegisterEffectFromString(
        CLSID_CustomFixBT2100SDREncodeEffect,
        pszXml,
        nullptr, // No custom properties
        0,       // No custom properties
        CreateFixBT2100SDREncodeImpl
        );
}

IFACEMETHODIMP FixBT2100SDREncodeEffect::Initialize(
    ...
    )
{
    ...
    try
    {
        ...
        data = reader->ReadData("DXRenderer\\FixBT2100SDREncodeEffect.cso");
    }
    ...
    HRESULT hr = pEffectContext->LoadPixelShader(GUID_FixBT2100SDREncodePixelShader, data->Data, data->Length);
...
HRESULT FixBT2100SDREncodeEffect::UpdateConstants()
{
...
IFACEMETHODIMP FixBT2100SDREncodeEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP FixBT2100SDREncodeEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
    return E_NOTIMPL;
}
...
IFACEMETHODIMP FixBT2100SDREncodeEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
    m_drawInfo = pDrawInfo;

    return m_drawInfo->SetPixelShader(GUID_FixBT2100SDREncodePixelShader);
}

// Calculates the mapping between the output and input rects.
IFACEMETHODIMP FixBT2100SDREncodeEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP FixBT2100SDREncodeEffect::MapInputRectsToOutputRect(
...
IFACEMETHODIMP FixBT2100SDREncodeEffect::MapInvalidRect(
    UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) FixBT2100SDREncodeEffect::GetInputCount() const
{
    return 1;
}
...
IFACEMETHODIMP_(ULONG) FixBT2100SDREncodeEffect::AddRef()
{
    m_refCount++;
    return m_refCount;
}

IFACEMETHODIMP_(ULONG) FixBT2100SDREncodeEffect::Release()
{
    m_refCount--;
...
// of the FixBT2100SDREncode interface, this method simply enables the developer
// to cast a FixBT2100SDREncode instance to an ID2D1EffectImpl or IUnknown instance.
IFACEMETHODIMP FixBT2100SDREncodeEffect::QueryInterface(
    _In_ REFIID riid,
