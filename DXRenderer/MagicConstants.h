...
static const float sc_DefaultHdrDispMaxNits = 750.0f; // Conservative default, prevents egregious behavior on low spec displays.
...
static const float sc_DefaultImageMaxCLL = 750.0f; // Needs more tuning based on real world content.
static const float sc_DefaultImageMedCLL = 80.0f; // Needs more tuning based on real world content (this is way too high).
static const float sc_MaxZoom = 2.5f; // Restrict max zoom to 1:1 scale.
